...
workflow chip {
	...
	Boolean keep_irregular_chr_in_bfilt_peak = true # when filtering with blacklist
	...
	Int macs2_mem_mb = 30000
	Int macs2_time_hr = 36
	...
	Array[File?] nodup_bams = dedup_bams 	# [rep_id]
	Array[File?] ctl_nodup_bams = inputBams # [rep_id]
	Array[File?] tas = []			# [rep_id]
	Array[File?] ctl_tas = []		# [rep_id]

	### other input types (peak)
	Array[Int?] fraglen = [200,200,200] 	# [rep_id]. fragment length if inputs are peaks
    ...
    meta {
        workflow_title : "ENCODE ChIP-Seq Pipeline"
        workflow_short_description : "Peak-calling and QC"
        workflow_long_description : "Use this workflow for performing peak-calling and QC for single-end ChIP-seq experiments with matched input/control samples."
    }
}

task organize_inputs {
	...
	output {
		Array[String] ordered_pulldowns = read_lines("ip_fq.txt")
		Array[String] ordered_inputs = read_lines("input_fq.txt")
	}
...
task idr {
	...
	command {
		...
		touch null 
                mv ${peak1} peak1_narrowpeak
                gzip peak1_narrowpeak
                mv peak1_narrowpeak.gz ${peak1}
                mv ${peak2} peak2_narrowpeak
                gzip peak2_narrowpeak
                mv peak2_narrowpeak.gz ${peak2}
                mv ${peak_pooled} pooled_narrowpeak
                gzip pooled_narrowpeak
                mv pooled_narrowpeak.gz ${peak_pooled}
